...
public class WarpController : MonoBehaviour
{
    ...
    //public GameObject flyingRaijinParticle;
    ...
    public void FlashToMark(Transform t, GameObject objectToMove, bool needStop)
    {
       
        ShowBody(false,objectToMove);
        if (needStop)
        {
            anim.speed = 0;
        }
       
        objectToMove.transform.DOMove(t.position, warpDuration).SetEase(Ease.InExpo).OnComplete(() => FinishFlash(objectToMove));
        blueTrail.Play();
        whiteTrail.Play();
    ...
    void FinishFlash(GameObject objectToMove)
    {
        ShowBody(true,objectToMove);
        ...
        //Instantiate(flyingRaijinParticle, transform.position, Quaternion.identity);
    ...
    void ShowBody(bool state, GameObject movingObject)
    {
        SkinnedMeshRenderer[] skinMeshList = movingObject.GetComponentsInChildren<SkinnedMeshRenderer>();
        foreach (SkinnedMeshRenderer smr in skinMeshList)
        {
            smr.enabled = state;
        }
